...
metadata$Sex <- with(metadata, ifelse(grepl("f", metadata$Sex,ignore.case = T, fixed = T), "Female", "Male"))

# Sort by sample Name
metadata <- metadata[order(metadata$Sample.Name),]

# Because some duplicates in the Samples remove them from Seqtab
seqtab.nochim <- seqtab.nochim[!(row.names(seqtab.nochim) %in% c("103", "318")), ]
...
ps_rare = rarefy_even_depth(ps, sample.size = 5573)
