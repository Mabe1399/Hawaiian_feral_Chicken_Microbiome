...
Combined_seqtab <- mergeSequenceTables(seqtab, seqtab2, repeats = "sum")
```
...
```{r , eval = F}
seqtab.nochim <- removeBimeraDenovo(Combined_seqtab, method = "consensus", multithread = T, verbose = T)

# Check dimension
dim(seqtab.nochim)

# Check chimera frequency
sum(seqtab.nochim) / sum(Combined_seqtab)
```
...
stat <- as.matrix(colSums(is.na(taxa.print)) / 28896 * 100)
