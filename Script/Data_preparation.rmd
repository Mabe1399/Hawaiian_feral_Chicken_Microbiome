...
dir.create(cut_output)

# We need to create a loop to remove in every files the primer
for (i in sample.names) {
  system2(cutadapt, "-g", FWD, "-G", REV, 
          "--discard-untrimmed", 
          "-o", file.path(cut_output, "${i}.trimmed_R1.fastq.gz"),
          "-p", file.path(cut_output, "${i}.trimmed_R2.fastq.gz"),
          file.path(Raw_Data, "${i}_R1.fastq.gz"),
          file.path(Raw_Data, "${i}_R2.fastq.gz"))
}
```
