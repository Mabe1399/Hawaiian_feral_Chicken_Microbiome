...
ps_dist_matrix <- phyloseq::distance(ps_Norm, method = "unifrac", weighted = F)
...
vegan::adonis2(ps_dist_matrix ~ phyloseq::sample_data(ps_Norm)$Island, permutations = 999)
```

```{r, eval = F}
# The Adonis test based on two variables
vegan::adonis2(ps_dist_matrix ~ phyloseq::sample_data(ps_Norm)$Island + phyloseq::sample_data(ps_Norm)$Sex, permutations = 999)
```
