...
```{r, eval = F}
kw <- kruskal.test(Richness ~ "any variable", data = alpha_div)
```
...
vegan::adonis2(ps_dist_matrix ~ phyloseq::sample_data(ps_Norm)$"variable", permutations = 999)
```

```{r, eval = F}
# The Adonis test based on two variables
vegan::adonis2(ps_dist_matrix ~ phyloseq::sample_data(ps_Norm)$"variable" + phyloseq::sample_data(ps_Norm)$"variable", permutations = 999)
```
