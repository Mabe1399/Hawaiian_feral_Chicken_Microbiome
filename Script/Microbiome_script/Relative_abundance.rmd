...
## Relative Abundance at genus level for the "core" microbiome

## Top Genus check

```{r, eval = F}
barplot_core_genus <- psmelt(ps_ASV_GWAS)
table(barplot_core_genus$Genus, useNA = "ifany") # table of phyla
barplot_core_genus$Genus[barplot_core_genus$Genus == ""] <- "NA" # change if there are empty names to NA
table(barplot_core_genus$Genus, useNA = "ifany") # check

# calculate sum of abundances
taxa_summary <- aggregate(barplot_core_genus$Abundance, by = list(Category = barplot_core_genus$Genus), FUN = sum)
taxa_summary <- taxa_summary[order(-taxa_summary$x), ]
taxa_summary
```

Print off top 10 Genus
...
```{r, eval = F}
ggplot(barplot_core_genus, aes(x = Sample, y = Abundance, order = Genus)) +
      ...
      theme(axis.text.x = element_text(angle = 90)) +
# scale_fill_manual(values=(color_list[c(10,6,8,5,4,3)])) +
      facet_grid(~Island, scales="free", space="free") + # change this per your liking!
      scale_y_continuous(expand = c(0,0)) +
      theme(strip.text.x = element_text(size = 8, color = "black"),
      #trip.background = element_rect(fill = "#ffffff"))
```
