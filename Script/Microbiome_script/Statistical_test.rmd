...
```{r, eval = F}
kw <- kruskal.test(InvSimpson ~ location.numeric, data = alpha_div)
kw
```
...
Spearman_covariate <- as.data.frame(alpha_div[, c(23:25, 27:28, 36)])
...
Spearman_covariate$InvSimpson <- as.numeric(Spearman_covariate$InvSimpson)
...
vegan::adonis2(ps_dist_matrix ~ phyloseq::sample_data(ps_Norm)$calc_sex, permutations = 999)
```
