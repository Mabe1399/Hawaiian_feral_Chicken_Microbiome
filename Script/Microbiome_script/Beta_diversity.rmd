...
## PCoA Plot Using Bray-Curtis (Sex)
...
# pcoa plot Giraffe Species
ggplot(g, aes(x = Axis.1, y = Axis.2, color = Sex, fill = Sex)) +
      theme_bw() +
      stat_ellipse(geom = "polygon", type = "t", alpha = 0.2, level = 0.9) + # confidence interval
      geom_point(color = "black", shape = 21, size = 2.5, stroke = 0.3) +
      xlab(paste0("PC1 (", tempx, "%)")) +
...
## PCoA Plot Using Bray-Curtis (Sample Type)
...
# pcoa plot
ggplot(g, aes(x = Axis.1, y = Axis.2, color = Sample.Type, fill = Sample.Type)) +
      theme_bw() +
      stat_ellipse(geom = "polygon", type = "t", alpha = 0.2, level = 0.9) + # confidence interval
      geom_point(color = "black", shape = 21, size = 2.5, stroke = 0.3) +
      xlab(paste0("PC1 (", tempx, "%)")) +
...
## PCoA Plot Using Bray-Curtis (location)

```{r}
ps_Norm_w_5_ind <- subset_samples(ps_Norm, ID %in% Metadata_loc_above_5_ind$ID)

ord_ps <- ordinate(ps_Norm_w_5_ind, "PCoA", "bray")
g <- plot_ordination(ps_Norm_w_5_ind, ord_ps, justDF = T)
head(g)
...
# pcoa plot
ggplot(g, aes(x = Axis.1, y = Axis.2, color = location.numeric, fill = location.numeric)) +
      theme_bw() +
      stat_ellipse(geom = "polygon", type = "t", alpha = 0.2, level = 0.9) + # confidence interval
      geom_point(color = "black", shape = 21, size = 2.5, stroke = 0.3) +
      xlab(paste0("PC1 (", tempx, "%)")) +
...
## PCoA Plot Using Bray-Curtis (Age Category)
```{r}
col_gir <- c("#a0be25", "#25be4b")

names(col_gir) <- c("Juvenile", "Adult")
...
# pcoa plot
ggplot(g, aes(x = Axis.1, y = Axis.2, color = Age_category, fill = Age_category)) +
      theme_bw() +
      stat_ellipse(geom = "polygon", type = "t", alpha = 0.2, level = 0.9) + # confidence interval
      geom_point(color = "black", shape = 21, size = 2.5, stroke = 0.3) +
      xlab(paste0("PC1 (", tempx, "%)")) +
...
## PCoA plot using Aitchinson (Island)
...
# pcoa plot Giraffe Species
ggplot(PCA_aitch, aes(x = Axis.1, y = Axis.2, color = Island, fill = Island)) +
      theme_bw() +
